...
type, public :: vertvisc_CS ; private
  ...
  real    :: alpha_gl90      !< Coefficient used to compute a depth-independent GL90 vertical
                             ...
                             ! Kv_GL90 corresponds to a KD_GL90 that scales as N^2 with depth.
                             !! [L2 T ~> m2 s]
  logical :: use_GL90_in_SSW        !< If true, use simpler method to calculate N^-2 in GL90 vertical
                             !viscosity ceofficient. This method is valid in in stacked shallow water mode.
  logical :: use_GL90_N2        !< If true, GL90 vertical viscosity coefficient that is depth-independent;
  ...
  integer :: id_au_gl90_vv = -1, id_av_gl90_vv = -1
  integer :: id_du_dt_visc_gl90 = -1, id_dv_dt_visc_gl90 = -1
...
!> Compute coupling coefficient associated with vertical viscosity parameterization as in Greatbatch and Lamb (1990),
!! Ferreira & Marshall (2006) and Zhao & Vallis (2008), hereafter referred to as the GL90 vertical
!! viscosity parameterization. This vertical viscosity scheme redistributes momentum in the vertical,
!! and is the equivalent of the Gent & McWilliams (1990) parameterization, but in a TWA (thickness-
!! weighted averaged) set of equations. The vertical viscosity coefficient nu is computed from kappa_GM
!! via thermal wind balance, and the following relation:
!! nu = kappa_GM * f^2 / N^2.
!! In the following subroutine kappa_GM is assumed either (a) constant or as (b) having an EBT structure.
  ...
  if (work_on_u) then
    ...
    do I=Isq,Ieq; if (do_i(I)) then
      ...
      do K=2,nz
        ...
        !    botfn determines when a point is within the influence of the GL90 bottom
...
subroutine vertvisc(u, v, h, forces, visc, dt, OBC, ADp, CDp, G, GV, US, CS, &
  ...
  do j=G%jsc,G%jec
    ...
    if ((CS%id_du_dt_visc_gl90 > 0) .or. (CS%id_KE_visc_gl90)) then
  ...
  do J=Jsq,Jeq
    ...
    if ((CS%id_dv_dt_visc_gl90 > 0) .or. (CS%id_KE_visc_gl90)) then
...
subroutine vertvisc_init(MIS, Time, G, GV, US, param_file, diag, ADp, dirs, &
  ...
  character(len=200) :: kappa_gl90_file
...
! Default, read and log parameters
  ...
  call get_param(param_file, mdl, "USE_GL90_IN_SSW", CS%use_GL90_in_SSW, &
                 "If true, use simpler method to calculate N^-2 in GL90 vertical "// &
  ...
  call get_param(param_file, mdl, "USE_GL90_N2", CS%use_GL90_N2, &
                 "If true, GL90 vertical viscosity coefficient that is depth-independent; "// &
                 "this corresponds to a kappa_GM that scales as N^2 with depth.", &
                 default=.false.)
  if (CS%use_GL90_N2) then
  ...
  call get_param(param_file, mdl, "READ_KD_GL90", CS%read_kappa_gl90, &
                 ...
                 "spatially varying kappa_gl90.", default=.false.)
  if (CS%read_kappa_gl90) then
    if (CS%kappa_gl90 > 0) then
        call MOM_error(FATAL, "vertvisc_init: KD_GL90 > 0 is not "// &
              "compatible with READ_KD_GL90 = TRUE. ")
    endif
    call get_param(param_file, mdl, "KD_GL90_FILE", kappa_gl90_file, &
                 "The file containing the spatially varying kappa_gl90.", &
                 default="INPUT/kd_gl90.nc")

    allocate(CS%kappa_gl90_2d(G%isd:G%ied, G%jsd:G%jed), source=0.0)
    call MOM_read_data(kappa_gl90_file, 'khth', CS%kappa_gl90_2d(:,:), G%domain, scale=US%m_to_L**2*US%T_to_s)
